...
namespace Utilities::General {
    ...
    inline QPoint interpolate(QPoint source, QPoint destination, float percentage) {
        float x = (1 - percentage) * source.x() + percentage * destination.x();
        float y = (1 - percentage) * source.y() + percentage * destination.y();
        return QPoint(static_cast<int>(x), static_cast<int>(y));
    }

    inline void getSubRectangle(uint8_t* destination, const uint8_t* source, int sourceWidth, int x, int y, int width, int height, int bytesPerPixel = 4) {
    ...
    inline void getSubRectangle(uint8_t* destination, const uint8_t* origin, int originalWidth, const QRect& rect, int bytesPerPixel = 4) {
        getSubRectangle(destination, origin, originalWidth, rect.x(), rect.y(), rect.width(), rect.height(), bytesPerPixel);
    }

    inline QRect clipRegionToWidgetBounds(QRect rect, const QSize& size) {
        if (rect.x() < 0) {
            rect.setX(0);
    ...
    inline Delta getImageCompressedDelta(uint8_t* source, int sourceWidth, const QRect& region, int bytesPerPixel = 4) {
